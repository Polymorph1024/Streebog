...
{
	// исходный вектор делим по 8 байт
...
{
	uint8_t t[64];		// промежуточный вектор
	if (CTX->buf_size < 64)
	{
		memset(t, 0x00, 64);	// обнул€ем промежуточный вектор
		memcpy(t, CTX->buffer, CTX->buf_size);	// пишем остаток сообщени€ в промежуточный вектор
		t[CTX->buf_size] = 0x01;	// добавл€ем в нужное место единичку
...
{
	memset(CTX, 0x00, sizeof(HashContext));	// ќбнул€ем все переменные базовой структуры
	if (hash_size == 256)
		memset(CTX->h, 0x01, 64);	// ƒлина хеша 256 бит
	else
		memset(CTX->h, 0x00, 64);	// ƒлина хеша 512 бит
	CTX->hash_size = hash_size;
	CTX->v_512[1] = 0x02;	// »нициализируем вектор v_512
}
...
static void Stage_3(HashContext *CTX)
{
	uint8_t t[64];
	memset(t, 0x00, 64);
	// формируем строку с размером сообщени€
	t[1] = ((CTX->buf_size * 8) >> 8) & 0xff;
	t[0] = (CTX->buf_size * 8) & 0xff;

	Padding(CTX); // дополн€ем оставшуюс€ часть до пoлных 64 байт

	g(CTX->h, CTX->N, (const uint8_t*)&(CTX->buffer));

	// формируем контрольную сумму сообщени€
	...
	memcpy(CTX->hash, CTX->h, 64);	// записываем результат в нужное место
}
...
void Update(HashContext *CTX, const uint8_t *m, size_t len)
{
	...
	while (len)
	{
		...
		if (CTX->buf_size == 64)
		{
			// ≈сли буфер заполнилс€ полностью, то делаем еще один второй этап
